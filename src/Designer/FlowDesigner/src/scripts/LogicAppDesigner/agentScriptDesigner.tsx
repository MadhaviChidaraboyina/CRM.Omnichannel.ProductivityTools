...
import * as AgentScriptUtils from "./utilities/agentScriptUtils";
...
cancelButton.innerHTML = AgentScriptUtils.AgentScriptUtils.getResourceString(SharedDefines.Constants.MACRO_ACTION_CANCEL);
    ...
    let _designerConfigPromise: Promise<SharedDefines.MacroDesignerConfig> = new Promise<SharedDefines.MacroDesignerConfig>(async function (resolve, reject) {
       let lcid = window.top.Xrm.Utility.getGlobalContext().getOrgLcid();
        ...
        if (macroCDSConfig.entities.length > 0) {
            ...
            designerConfig.SearchHint = AgentScriptUtils.AgentScriptUtils.getResourceString("LADESIGNER_SEARCHTEXT");;
            ...
            if (!baseUrl) {
                if (!isNullOrUndefined(config.msdyn_designerfallbackurl)) {
                    ...
                    try {
                        ...
                        if (DesignerConstants.Constants.gccDataCenter.indexOf(crmDataCenter) != -1) {
                            baseUrl = DesignerConstants.Constants.fairfaxFallbackURL;
                        } else {
                            baseUrl = DesignerConstants.Constants.publicFallbackURL;
                        }
                    } catch (error) {
                        let obj: SharedDefines.LogObject = {
                            level: SharedDefines.LogLevel.Warning,
...
operKindDisplayText[SharedDefines.Kind.Action] = AgentScriptUtils.AgentScriptUtils.getResourceString("DESIGNER_ACTION");
operKindDisplayText[SharedDefines.Kind.Trigger] = AgentScriptUtils.AgentScriptUtils.getResourceString("DESIGNER_TRIGGER");
...
async function startDesigner(rpc) {
    try {
        ...
        try {
            ...
            designerOptions.SearchHint = AgentScriptUtils.AgentScriptUtils.getResourceString("LADESIGNER_SEARCHTEXT");
            designerOptions.UserVoiceMessage = designerConfig.UserVoiceText || AgentScriptUtils.AgentScriptUtils.getResourceString("DESIGNER_USERVOICEMSG");
        ...
        saveButton.innerHTML = AgentScriptUtils.AgentScriptUtils.getResourceString(SharedDefines.Constants.MACRO_ACTION_SAVE_AND_CLOSE);
        if (saveButton) {
...
async function getCRMData() {
    ...
    try {
        let result = await (window.top as any).Xrm.WebApi.retrieveMultipleRecords("msdyn_sessiontemplate", "?$filter=msdyn_sessiontemplateid eq '" + sessionTemplateId +"'&$expand=msdyn_msdyn_prod_agentscript_msdyn_sessiontemplat($select=msdyn_name,msdyn_productivityagentscriptid,msdyn_description)&$select=msdyn_name");   
        result.entities.forEach(function (item) {
            item.msdyn_msdyn_prod_agentscript_msdyn_sessiontemplat.forEach(function (input) {
                let agentScriptItem: SharedDefines.ListDynamicValue= {
                    value: input.msdyn_productivityagentscriptid,
