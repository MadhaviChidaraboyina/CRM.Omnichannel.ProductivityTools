...
module MscrmControls.ProductivityToolPanel
{
	...
	export class SessionChangeManager
	{
        ...
        {
            try {
                let currentSessionId = event._eventArgs._inputArguments.newSessionId;
                let previousSessionId = event._eventArgs._inputArguments.previousSessionId;
        ...
        {
            try {
                let createdSessionId = event._eventArgs._inputArguments.sessionId;
        ...
        {
            try {
                let closedSessionId = event._eventArgs._inputArguments.sessionId;
		...
		public getCurrentFocusedSessionId(): string
		{
			...
			try
			{
				return windowObject.Xrm.App.sessions.getFocusedSession().sessionId;
			}
			catch (e)
			{
                console.log("error occured" + e);
