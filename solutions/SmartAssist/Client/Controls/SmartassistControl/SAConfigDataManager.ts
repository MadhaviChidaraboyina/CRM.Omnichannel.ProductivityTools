    ...
    export class SAConfigDataManager {
        ...
        private async fetchSAConfigurationsData() {
            ...
            try {
                ...
                var result = await SmartassistControl._context.webAPI.retrieveMultipleRecords(this.saConfigSchema.EntityName, fetchXml) as any;
                let _util = new Utility(SmartassistControl._context);
                for (var i = 0; i < result.entities.length; i++) {
                    this.saConfig.push(new SAConfig(result.entities[i], this.saConfigSchema, this.acConfigSchema.AdaptiveCardTemplateAlias, _util, SmartassistControl._context))
                }
            } catch (error) {
