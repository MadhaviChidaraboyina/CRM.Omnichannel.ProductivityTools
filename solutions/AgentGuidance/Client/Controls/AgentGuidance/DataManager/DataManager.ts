...
module MscrmControls.ProductivityToolAgentGuidance {
    ...
    export class DataManager {
        ...
        public fetchAgentScriptRecords(SessionTemplateId: string) {
            ...
            let isApiCalled: any = StateManager.getState(LocalStorageKeyConstants.dbCall + SessionTemplateId);

            if (isApiCalled == null) {

                StateManager.SetState(LocalStorageKeyConstants.dbCall + SessionTemplateId, SessionTemplateId);
                ...
                agentScriptDataPromise.then((isAgentScriptFound: boolean) => {
                    ...
                    StateManager.SetState(SessionTemplateId + LocalStorageKeyConstants.isAgentScriptFound, IsAgentScriptAvailable);
                    StateManager.DeleteState(LocalStorageKeyConstants.dbCall + SessionTemplateId);
        ...
        public fetchSmartAssistRecords(liveWorkStreamId: string) {
            ...
            let isApiCalled: any = StateManager.getState(LocalStorageKeyConstants.dbCall + liveWorkStreamId);

            if (isApiCalled == null) {

                StateManager.SetState(LocalStorageKeyConstants.dbCall + liveWorkStreamId, liveWorkStreamId);
                ...
                smartAssistDataPromise.then((isSmartAssistBotFound: boolean) => {
                    ...
                    StateManager.SetState(liveWorkStreamId + LocalStorageKeyConstants.isSmartAssistFound, isSmartAssistAvailable);
                    StateManager.DeleteState(LocalStorageKeyConstants.dbCall + liveWorkStreamId);
                    this.context.utils.requestRender();
                },
                    (errorMessage: string) => {
