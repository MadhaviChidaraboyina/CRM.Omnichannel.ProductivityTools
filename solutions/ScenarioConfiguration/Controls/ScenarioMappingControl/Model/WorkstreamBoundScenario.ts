...
module MscrmControls.Service.CIProvider {
	...
	export class WorkstreamBoundScenario implements IEntityBoundScenario {

		ScenarioEntityFilter: string = "?$filter=_msdyn_workstreamid_value eq {0}&$expand=msdyn_NotificationTemplate($select=msdyn_name),msdyn_SessionTemplate($select=msdyn_name),msdyn_ParentScenarioId($select=msdyn_displayname)";
		WorkstreamEntityName = "msdyn_liveworkstream";
				...
				ODataUtility.GetScenariosForEntity(context, this.ScenarioEntityFilter, entityId, templateType).then(
					(resp) => {
						...
						resolve({
							...
							ViewId: viewId,
							BoundEntityName: this.WorkstreamEntityName,
