...
namespace Microsoft.CIFramework.Internal {
	 ...
	 *
	*/
    export function renderEventNotification(header:any,body:any,actions:any,notificationType:any,panelWidth:any,eventType:any): Map<any,any>{
	...
	*/
	 ...
	 *
	*/
    export function notifyEventClient(notificationUX: Map<string,Map<string,any>>,panelWidth: any): Promise<any>{
       	let widgetIFrame = (<HTMLIFrameElement>listenerWindow.document.getElementById(Constants.widgetIframeId));
		let toastDiv =  widgetIFrame.contentWindow.document.getElementById("toastDiv");
		let i = 0;
		...
		let map = new Map();
		map = renderEventNotification(header,body,actions,notificationType,panelWidth,eventType);
		if(actions != null && actions != "undefined"){
			for( i = 0; i < actions.length; i++){
				for (let key in actions[i]) {
