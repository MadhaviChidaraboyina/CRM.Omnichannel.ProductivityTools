...
namespace Microsoft.CIFramework.Internal {
	...
	export class UCINotificationTemplate {
        ...
        /*public instantiateTemplate(templateParams: any): Promise<any> { //TODO: The return type here will change based on platform definition
            return new Promise<any>(function (resolve: (value?: Map<string, string> | PromiseLike<Map<string, string>>) => void, reject: (error: Error) => void) {
                try {   //TODO: Parameterized apptab title
                    let ret: Map<string, any> = new Map<string, string>();
                    ...
                    for (let i in this.infoFields) {
                        stringResolvers.push(TemplatesUtility.resolveTemplateString(this.infoFields[i]["msdyn_value"], templateParams, this.name).then(
                            function (result: string) {
                                ret[this.infoFields[i]["msdyn_lineheader"]] = result;
                            }.bind(this),
                            function (error: Error) {
                                ret[this.infoFields[i]["msdyn_lineheader"]] = this.infoFields[i]["msdyn_value"];
                                //TODO: log error
                            }.bind(this)));
                    }
                    ret["icon"] = this.icon;
                    ret["timeout"] = this.timeout;
                    ret["actionButtons"] = this.actionButtons;
