...
export class OrgDynamicsCrmStartPage extends BasePage {
  ...
  public async navigateToOrgUrl() {
    await Promise.all([
      ...
      this.Page.goto(this._orgUrl + "/apps"),
  ...
  private async _signIn(email: string, pwd: string) {
    ...
    await this.waitUntilSelectorIsVisible(
      SelectorConstants.SignInEmailInput,
      Constants.Three,
      this.Page,
    ...
    await this.Page.fill(SelectorConstants.SignInEmailInput, email);
    await this.waitUntilSelectorIsVisible(
      SelectorConstants.SignInNextButton,
      Constants.Three,
      this.Page,
      Constants.MaxTimeout
    );
    ...
    await this.waitUntilSelectorIsVisible(
      SelectorConstants.SignInPasswordInput,
      Constants.Three,
      this.Page,
      Constants.MaxTimeout
    );
    ...
    await this.Page.fill(SelectorConstants.SignInPasswordInput, pwd);
    await this.waitUntilSelectorIsVisible(
      SelectorConstants.SignInSignInButton,
      Constants.Three,
      this.Page,
      Constants.MaxTimeout
    );
    ...
    const staysignedin = await this.waitUntilSelectorIsVisible(
      SelectorConstants.StaySignedInPopup,
      Constants.Three,
      this.Page,
      Constants.MaxTimeout
    );
  ...
  private async validateLandingPage() {
    await this.waitUntilSelectorIsVisible(
      SelectorConstants.HomePageButton,
      Constants.Five,
      this.Page,
      Constants.MaxTimeout
    );
    const homePageButton = await this.Page.waitForSelector(
      SelectorConstants.HomePageButton
    );
  ...
  public async goToOrgUrlAndSignIn(login: string, password: string) {
    ...
    await this._page
      .waitForSelector(OrgDynamicsCrmBasePageConstants.EmailInput)
      .catch((error) => {
        throw new Error(
          `Can't verify that Sign in window contains input field for email, phone or Skype. Inner exception: ${error.message}`
        );
      });
    await this._page.fill(OrgDynamicsCrmBasePageConstants.EmailInput, login);
    await this._page.waitForTimeout(
      MicrosoftSignInPageConstants.DefaultFillTimeout
    );
    await this._page
      .waitForSelector(OrgDynamicsCrmBasePageConstants.NextButton)
      .catch((error) => {
        throw new Error(
          `Can't verify that Sigh in window contains Next button. Inner exception: ${error.message}`
        );
      });
    ...
    await this._page
      .waitForSelector(OrgDynamicsCrmBasePageConstants.PasswordInput)
      .catch((error) => {
        throw new Error(
          `Can't verify that Enter password window contains input field for password. Inner exception: ${error.message}`
        );
      });
    await this._page.fill(
      OrgDynamicsCrmBasePageConstants.PasswordInput,
      password
    );
    await this._page.waitForTimeout(
      MicrosoftSignInPageConstants.DefaultFillTimeout
    ...
    await this._page
      .waitForSelector(OrgDynamicsCrmBasePageConstants.SignInButton)
      .catch((error) => {
        throw new Error(
          `Can't verify that Enter password window contains Sign in button. Inner exception: ${error.message}`
        );
      });
    ...
    await this._page
      .waitForSelector(OrgDynamicsCrmBasePageConstants.NoSaveButton)
      .catch((error) => {
        throw new Error(
          `Can't verify that Stay signed in window contains No button. Inner exception: ${error.message}`
        );
      });
  ...
  public async goToOrgUrl() {
    await this._page.goto(
      TestSettings.OrgUrl + "/main.aspx?forceUCI=1&pagetype=apps"
    );
  ...
  public async navigateToSiteMap() {
    await this.Page.waitForSelector(
      SelectorConstants.AgentScriptMenuItem.replace("{0}", "Overview")
    );
  ...
  public async goToMyApp(appName: string) {
    ...
    // });
    const iframe = await this._page.waitForSelector(
      `//iframe[@id="AppLandingPage"]`
    );
    ...
    await this._page.waitForTimeout(Constants.DefaultTimeout);
    await this.waitForDomContentLoaded();
    const chatPopup = await this.waitUntilSelectorIsVisible(SelectorConstants.GetstartedPopup);
    if (chatPopup) {
      await this._page.click(SelectorConstants.CancelGetstarted);
    }
  }

  public async waitUntilFrameSelectorIsVisible(
    selectorVal: string,
    frame: any,
  ...
  public async goToCSWAppDesigner() {
    const iframe = await this._page
      .waitForSelector(`//iframe[@id="AppLandingPage"]`)
      .catch((error) => {
        throw new Error(
          `Can't verify that current page is App Landing Page. Inner exception: ${error.message}`
        );
      });
    const iframeNodes = await iframe.contentFrame();
    await iframeNodes.click(
      `[title="More Options"][role="button"][data-lp-id="msdyn_CustomerServiceWorkspace0-options"]`
    );
  ...
  public async addSlugOrSessionConditionForExpressionBuilder(
    LhsValue: any,
    RhsValue: any
  ) {
    ...
    await this.basepage.waitUntilSelectorIsVisible(
      MacrosConstants.ConditionOperatorValueSelector,
      MacrosConstants.Five,
      iframe,
      MacrosConstants.MaxTimeout
    );
    ...
    await this.basepage.waitUntilSelectorIsVisible(
      MacrosConstants.CustomerServiceSelector,
      MacrosConstants.Three,
      iframe,
      MacrosConstants.OpenWsWaitTimeout
    );
    ...
    await this.basepage.waitUntilSelectorIsVisible(
      MacrosConstants.ShowOptionSelector,
      MacrosConstants.Three,
      iframe,
      MacrosConstants.OpenWsWaitTimeout
    );
    ...
    await this.basepage.waitUntilSelectorIsVisible(
      MacrosConstants.TextSelector.replace(
        "{0}",
        MacrosConstants.AgentScriptName
      ),
      MacrosConstants.Three,
      iframe
    );
    ...
    await this.basepage.waitUntilSelectorIsVisible(
      MacrosConstants.TextSelector.replace(
        "{0}",
        MacrosConstants.AgentscriptName2
      ),
      MacrosConstants.Three,
      iframe
    );
    ...
    await parentIframe.waitForSelector(
      MacrosConstants.SaveExpresionBuilderBtnSelector
    );
    await parentIframe.$eval(
      MacrosConstants.SaveExpresionBuilderBtnSelector,
      (el) => {
        (el as HTMLElement).click();
      }
  ...
  public async navigateToAgentExperienceOverview() {
    await this.waitUntilSelectorIsVisible(
      MacrosConstants.AgentScriptOverview,
      MacrosConstants.Three,
      this.Page
    );
