...
function Get-MsBuildPath([int] $MaxVersion = 2017)
{
    ...
    $VS2019 = "$Env:programfiles (x86)\Microsoft Visual Studio\2019\Enterprise\MSBuild\Current\Bin\MSBuild.exe"
    ...
    If ((2019 -le $MaxVersion) -And (Test-Path $VS2019)) { return $VS2019 } 

    throw "Unable to find msbuild"
}

Function Get-MsBuildLocation() {
    $msbuildLocations = @("${env:ProgramFiles(x86)}\Microsoft Visual Studio\2019\Enterprise\MSBuild\Current\Bin\MSBuild.exe",
...
& "${env:ProgramFiles(x86)}\Microsoft Visual Studio\2019\Enterprise\MSBuild\Current\Bin\MSBuild.exe" $buildFilePath /target:$target /verbosity:m /m:1
